...
import CartDrawer from '@/components/CartDrawer'; // Import du nouveau panier centralisé
...
const THEME = {
  ...
  shadow: 'group-hover:border-blue-500/30'
};
...
export default function PersoPage() {
  ...
  const handleAddToCart = (product: any) => {
    ...
    addToCart({
      ...
      qty: Number(sel.qty), 
      category: 'Impression' // Catégorie pour le panier
    });
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0f092e] flex items-center justify-center">
      <p className="font-black text-blue-500 tracking-[0.5em] uppercase animate-pulse italic text-xs">Chargement Impression...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0f092e] text-white font-sans flex flex-col relative overflow-x-hidden">
      ...
      <header className="py-6 px-6 border-b border-white/10 flex justify-between items-center bg-[#0f092e]/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <button onClick={() => setIsCartOpen(true)} className="relative group p-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-white/40 group-hover:text-blue-500 transition-colors">
                Panier ({cart.length})
            </span>
        </button>
      </header>
                  ...
                  <div className="space-y-5">
                    {product.hasVariants && (
                       <div className="space-y-2">
                          <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em]">Options</p>
                          <select 
                            value={sel.variant} 
                            onChange={(e) => updateSelection(product.id, 'variant', e.target.value)} 
                            className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-[11px] font-black uppercase text-white outline-none focus:border-blue-500 transition-colors cursor-pointer"
                          >
                    ...
                    <div className="flex items-center justify-between pt-6 mt-2 border-t border-white/10">
                      <div>
                        <div className="flex items-baseline gap-1">
                          ...
                          <span className="text-[10px] text-blue-500/60 font-black italic">HT</span>
                        </div>
                        <p className="text-[8px] text-white/20 font-bold uppercase tracking-widest mt-1">{(currentTotal / sel.qty).toFixed(2)}€ / unité</p>
                      </div>
