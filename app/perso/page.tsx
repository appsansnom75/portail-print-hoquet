...
const PRODUCTS_CONFIG = [
  {
    ...
    prices: { default: [8.45, 8.00, 7.65, 7.55, 7.40, 6.90] } // Prix unitaire par lot de 500
  },
  {
    id: 'calendrier',
    name: 'Calendrier',
    ...
    quantities: [500, 1000, 2000, 3000, 5000, 10000, 15000, 20000],
    prices: {
  ...
  },
  {
    ...
    quantities: [100, 200, 300, 400, 1000, 2000, 3000, 4000],
...
export default function PersoPage() {
  ...
  const calculateTotalPrice = (product: any) => {
    ...
    const basePricePerUnit = priceList[qtyIndex] || priceList[0];
    
    // Le calcul : prix unitaire (qui correspond en fait au prix du lot de base) * (quantité choisie / quantité de base)
    // Mais pour simplifier selon tes données, on considère que le prix affiché est celui pour cette quantité
    return basePricePerUnit * (sel.qty / product.quantities[0]);
  };
  ...
  const handleAddToCart = (product: any) => {
    ...
    addItemToGlobalCart({
      ...
      price: total / sel.qty, // On repasse au prix unitaire pour le context
      qty: sel.qty,
      category: 'Impression'
    });
    ...
    <div className="min-h-screen bg-[#0f092e] text-white font-sans flex flex-col relative">
            ...
            return (
              <div key={product.id} className="flex flex-col group">
                ...
                <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col h-full">
                  <h3 className="font-black text-xl uppercase tracking-tighter text-blue-500 mb-6 leading-tight">{product.name}</h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-white/5 text-center">
                    <p className="font-black text-2xl mb-3 text-white">{totalPrice.toFixed(2)}€ <span className="text-[10px] text-white/40">HT</span></p>
      ...
      <div className="fixed bottom-8 left-8 z-[100]">
        <button onClick={() => setIsCartOpen(!isCartOpen)} className="bg-white text-[#0f092e] px-8 py-4 rounded-full font-black uppercase text-[10px] tracking-widest shadow-2xl flex items-center gap-4 hover:bg-blue-600 hover:text-white transition-all">
          Panier Impression {cart.length > 0 && <span className="bg-blue-500 text-white px-2 py-0.5 rounded text-[9px]">{cart.length}</span>}
        </button>
        {isCartOpen && (
