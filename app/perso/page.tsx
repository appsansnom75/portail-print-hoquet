...
export default function PersoPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f092e] text-white p-8 font-sans">
      {/* BOUTON DE SECOURS - Si tu ne vois pas ce bouton, Vercel n'a pas mis à jour ton code */}
      <div className="max-w-7xl mx-auto mb-8">
        <button 
          onClick={() => { localStorage.clear(); clearCart(); window.location.reload(); }}
          className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shadow-xl"
        >
          🔄 Réinitialiser le cache du site
        </button>
      </div>
      ...
      <header className="max-w-7xl mx-auto mb-12 flex justify-between items-center">
        ...
        <h1 className="text-xl font-black uppercase tracking-tighter text-blue-500 underline decoration-blue-500/30">Configuration v2.0</h1>
      </header>
          ...
          return (
            <div key={product.id} className="bg-white/5 border border-white/10 p-6 rounded-3xl flex flex-col">
              ...
              <div className="space-y-4 mb-8">
                {product.hasVariants && (
                  <select 
                    ...
                    className="w-full bg-white/10 p-3 rounded-xl text-xs font-bold outline-none border border-white/5"
                  >
                ...
                <select 
                  ...
                  className="w-full bg-white/10 p-3 rounded-xl text-xs font-bold outline-none border border-blue-500/20"
                >
              ...
              <div className="mt-auto pt-6 border-t border-white/10 flex justify-between items-center">
                ...
                <button onClick={() => handleAddToCart(product)} className="bg-white text-[#0f092e] px-6 py-3 rounded-xl font-black uppercase text-[10px] hover:bg-blue-500 hover:text-white transition-all">Ajouter</button>
              </div>
            </div>
          );
        })}
      ...
      {isCartOpen && (
        <div className="fixed bottom-8 right-8 w-80 bg-white text-black rounded-3xl shadow-2xl z-50 overflow-hidden border border-slate-200">
          <div className="p-4 bg-slate-50 flex justify-between items-center border-b">
            <span className="font-black text-[10px] uppercase tracking-widest text-slate-500">Panier</span>
            <button onClick={() => setIsCartOpen(false)} className="text-red-500 font-bold px-2">✕</button>
          </div>
          <div className="p-4 max-h-60 overflow-y-auto space-y-3">
            {cart.map(item => (
              <div key={item.id} className="flex justify-between items-center border-b border-slate-100 pb-2">
                <div className="text-[10px] font-bold uppercase w-2/3">{item.name}</div>
                <div className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-1 rounded">{item.qty} ex.</div>
              </div>
            ))}
          </div>
          <div className="p-4">
            <Link href="/panier" className="block w-full bg-[#0f092e] text-white text-center py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em]">Finaliser</Link>
          </div>
        </div>
      )}
    </div>
