...
export default function HistoriqueCommandes() {
  ...
  const confirmReorder = async () => {
    ...
    try {
      ...
      fetchOrders();
    } catch (err) { alert("Erreur"); } finally { setIsReordering(false); }
  };
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center md:items-end border-b border-white/10 pb-8 gap-6">
            ...
            <div className="flex gap-3">
              ...
              <Link href="/" className="text-[9px] font-black uppercase bg-white/5 border border-white/10 px-6 py-4 rounded-2xl hover:bg-white/10 transition-all text-center">Boutique</Link>
            </div>
        </div>
      ...
      <AnimatePresence>
        {orderToReorder && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#0f092e]/90 backdrop-blur-md">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white text-[#0f092e] w-full max-w-md rounded-[40px] p-10 space-y-6 shadow-2xl">
              ...
              <div className="bg-gray-100 rounded-3xl p-6">
                 ...
                 <div className="text-[10px] font-bold uppercase opacity-40 border-t border-gray-200 pt-4">📍 {orderToReorder.delivery_address}</div>
              </div>
