...
export default async function AdminCartsPage() {
  ...
  // 2. Vérifie le rôle super_admin via service_role (jamais exposé au front)
  ...
  // 4. Commandes En attente depuis plus de 3 jours
    ...
    <div className="p-8 max-w-6xl mx-auto font-sans">
      ...
      <section className="mb-14">
        <h2 className="text-xl font-semibold mb-1">
          Paniers en cours non validés
          <span className="ml-2 text-sm font-normal text-gray-500">
            ({drafts?.length ?? 0})
          </span>
        </h2>
        <p className="text-sm text-gray-400 mb-4">
          Agences qui ont commencé une commande sans la valider
        </p>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-blue-50 text-left">
                ...
                <th className="p-3 border">Articles dans le panier</th>
      ...
      <section>
        ...
        <p className="text-sm text-gray-400 mb-4">
          Commandes validées par les agences mais pas encore traitées
        </p>
            ...
            <tbody>
              {pending.map((order) => {
                ...
                return (
                  <tr
                    key={order.id}
                    className={days > 7 ? 'bg-red-50' : 'hover:bg-gray-50'}
                  >
