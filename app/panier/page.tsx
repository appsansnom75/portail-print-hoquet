...
export default function CartPage() {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      clearCart();
    } catch (error) { alert("Erreur envoi."); } finally { setIsSubmitting(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f092e] text-white font-sans pb-20">
      ...
      <header className="py-8 px-6 max-w-6xl mx-auto flex justify-between items-center border-b border-white/10 mb-10">
          <Link href="/" className="text-[10px] font-black uppercase opacity-40 hover:opacity-100 transition-opacity flex items-center gap-2">
            <span>←</span> Retour Boutique
          </Link>
          <h1 className="text-[10px] font-black uppercase tracking-widest italic text-blue-500">Finaliser la commande</h1>
      ...
      <main className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-blue-600/10 border border-blue-500/20 rounded-[40px] p-8">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-[11px] font-black text-blue-400 uppercase">{agencyData?.name}</div>
                <select value={selectedUser} onChange={(e) => setSelectedUser(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl p-4 text-[10px] font-black outline-none uppercase text-white cursor-pointer">
             ...
             <div className="space-y-4">
                {cart.length === 0 ? <p className="text-[10px] opacity-40 uppercase">Vide.</p> : cart.map((item) => (
                    <div key={item.id} className="flex justify-between items-center border-b border-white/5 pb-4 last:border-0 last:pb-0">
                      <div className="flex flex-col"><span className="text-[12px] font-black uppercase">{item.name}</span><span className="text-[10px] text-white/50 uppercase font-bold">x{item.qty}</span></div>
                      <div className="flex items-center gap-4"><span className="text-[12px] font-bold">{(item.price * item.qty).toFixed(2)}€</span><button onClick={() => removeFromCart(item.id)} className="text-[10px] text-red-500 font-bold hover:scale-110">(X)</button></div>
                    </div>
                ))}
             </div>
          </section>
          ...
          <section className="bg-white/5 border border-white/10 rounded-[40px] p-8 space-y-4">
            ...
            <textarea placeholder="ADRESSE" value={address} onChange={(e) => setAddress(e.target.value)} className="w-full h-24 bg-black/40 border border-white/5 rounded-2xl p-4 text-[10px] font-black outline-none uppercase text-white resize-none" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input type="email" placeholder="EMAIL" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl p-4 text-[10px] font-black outline-none uppercase text-white" />
              <input type="tel" placeholder="TÉLÉPHONE" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl p-4 text-[10px] font-black outline-none uppercase text-white" />
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-10 rounded-[40px] text-[#0f092e] sticky top-10 shadow-2xl">
            <h2 className="text-[10px] font-black uppercase mb-4 opacity-40">Total HT</h2>
            <span className="text-5xl font-black italic tracking-tighter">{totalHT.toFixed(2)}€</span>
            <button onClick={() => setShowConfirm(true)} disabled={cart.length === 0} className="w-full mt-10 py-6 bg-[#0f092e] text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-blue-600 transition-all active:scale-95 disabled:opacity-20">Vérifier la commande</button>
          </div>
        </div>
      </main>
        ...
        {showConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#0f092e]/95 backdrop-blur-xl">
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white text-[#0f092e] w-full max-w-lg rounded-[40px] p-10 space-y-6">
              ...
              <div className="text-center py-4 border-y border-gray-100 space-y-2">
                    <p className="text-[11px] font-black uppercase">{agencyData?.name}</p>
                    <p className="text-[10px] font-bold opacity-40 uppercase">Par : {selectedUser}</p>
                    <p className="text-3xl font-black italic mt-4">{totalHT.toFixed(2)}€ HT</p>
              </div>
              <button onClick={handleFinalSubmit} disabled={isSubmitting} className="w-full py-6 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-xl">
                {isSubmitting ? "..." : "Confirmer et Envoyer"}
              </button>
              <button onClick={() => setShowConfirm(false)} className="w-full text-[9px] font-black uppercase opacity-20 text-center">Retour au panier</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
