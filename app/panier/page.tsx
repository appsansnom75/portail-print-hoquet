...
export default function CartPage() {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      const { error } = await supabase.from('orders').insert([{
        agency_name: agencyData.name, client_email: email, client_phone: phone,
        delivery_address: address, produits_liste: cart.map(item => item.name).join(', '),
        quantite_liste: cart.map(item => item.qty).join(', '), total_ht: totalHT,
        instructions: `Commandé par : ${selectedUser} -- ${instructions}`, status: 'En attente'
      }]);
      if (error) throw error;
      setOrderSent(true);
      setShowConfirm(false);
      clearCart();
    } catch (err) { console.error(err); alert("Erreur lors de l'envoi."); } finally { setIsSubmitting(false); }
  };
  ...
  if (orderSent) return (
    <div className="min-h-screen bg-[#0f092e] flex flex-col items-center justify-center text-center p-6 italic font-black uppercase text-white">
      ...
      <p className="text-blue-500 text-xs tracking-widest mb-12">Merci à l'équipe {agencyData?.name}</p>
  ...
  return (
    <div className="min-h-screen bg-[#0f092e] text-white font-sans pb-20">
      ...
      <main className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-white/5 border border-white/10 rounded-[40px] p-8 space-y-4">
            ...
            <textarea placeholder="ADRESSE DE LIVRAISON" value={address} onChange={(e) => setAddress(e.target.value)} className="w-full h-24 bg-black/40 border border-white/5 rounded-2xl p-4 text-[10px] font-black outline-none focus:border-blue-500/50 resize-none uppercase text-white" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      {/* MODAL DE CONFIRMATION REPRIS INTEGRALEMENT */}
      <AnimatePresence>
        {showConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#0f092e]/95 backdrop-blur-xl">
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white text-[#0f092e] w-full max-w-lg rounded-[40px] p-10 space-y-6">
              ...
              <div className="text-center py-2"><span className="text-4xl font-black italic">{totalHT.toFixed(2)}€ HT</span></div>
              <button onClick={handleFinalSubmit} disabled={isSubmitting} className="w-full py-6 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-xl shadow-blue-500/20">{isSubmitting ? "Envoi..." : "Confirmer et Envoyer"}</button>
