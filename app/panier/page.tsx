...
export default function CartPage() {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      // 1. RÉCUPÉRATION DU NUMÉRO DE COMMANDE
      ...
      // Sécurité : si le compteur n'existe pas ou bug, on utilise un timestamp (ex: 1713032400)
      let nextOrderId = counterData?.last_value ? counterData.last_value + 1 : Math.floor(Date.now() / 100000);

      // 2. TENTATIVE DE MISE À JOUR (on continue même si ça échoue à cause du RLS)
      try {
        await supabase.from('config')
          .update({ last_value: nextOrderId })
          .eq('counter_name', 'order_id');
      } catch (e) { 
        console.warn("Échec mise à jour compteur (RLS ?), mais on continue l'envoi."); 
      }
      ...
      // 3. INSERTION DANS LA TABLE ORDERS (Optionnel si tu n'utilises que Make)
      try {
          await supabase.from('orders').insert([{
            order_number: nextOrderId, 
            agency_name: agencyData?.name || "Agence Inconnue",
            client_name: selectedUser,
            items: itemsFormatted,
            total_ht: totalHT,
            delivery_address: `${address}, ${zipCode} ${city}`,
            status: 'En attente'
          }]);
      } catch (e) { console.error("Erreur insertion table orders", e); }

      // 4. ENVOI CRUCIAL VERS MAKE
      const response = await fetch('https://hook.eu1.make.com/mb6ok4o2jv41vrhd37r101wi98b1lfz4', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          order_number: nextOrderId, // C'EST CETTE VARIABLE QUI REMPLIT TON GH-
          agency_name: agencyData?.name || "Inconnu",
          client_name: selectedUser,
          items: itemsFormatted,
          total_ht: totalHT,
      ...
      console.error(err);
      alert("Erreur lors de la transmission. Vérifiez votre connexion.");
    } finally {
      setIsSubmitting(false);
    }
  };

  if (loading) return <div className="min-h-screen bg-[#0f092e] flex items-center justify-center text-white uppercase text-[10px] animate-pulse italic">Chargement du profil...</div>;
  ...
  if (orderSent) return (
    <div className="min-h-screen bg-[#0f092e] flex flex-col items-center justify-center text-center p-6 text-white font-black italic uppercase">
      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
        <h2 className="text-6xl mb-6 text-blue-500 tracking-tighter">Commande Transmise</h2>
        <p className="text-[10px] mb-12 opacity-50">Le bon de production est en cours de génération.</p>
      ...
      <AnimatePresence>
        {showConfirm && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white text-[#0f092e] w-full max-w-2xl rounded-[60px] p-14 space-y-8 shadow-2xl overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-2 bg-blue-600"></div>
              <h3 className="text-3xl font-black uppercase italic text-center text-blue-600 tracking-tighter">Confirmation Finale</h3>
              <p className="text-center text-[10px] font-black opacity-40 uppercase italic">Veuillez vérifier vos informations avant l'envoi en production.</p>
              ...
              <div className="bg-gray-50 rounded-[35px] p-8 space-y-4">
                ...
                <div className="border-t border-gray-200 pt-4">
                    <p className="text-[9px] font-black opacity-40 uppercase italic mb-1">Livré à :</p>
                    <p className="text-[11px] font-black uppercase">{address}, {zipCode} {city}</p>
                </div>
              </div>
              ...
              <button onClick={() => setShowConfirm(false)} className="w-full text-[9px] font-black uppercase opacity-30 italic hover:opacity-100">Modifier la saisie</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
