    ...
    <div className="min-h-screen bg-[#0f092e] text-white font-sans pb-20">
      ...
      <main className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="space-y-4">
              {cart.map((item) => (
                <div key={item.id} className="flex justify-between items-center border-b border-white/5 pb-4 last:border-0">
                  ...
                  <div className="flex items-center gap-4"><span className="text-[12px] font-bold">{(item.price * item.qty).toFixed(2)}€</span><button onClick={() => removeFromCart(item.id)} className="text-[10px] text-red-500 font-bold hover:scale-110 transition-transform">(X)</button></div>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-10 rounded-[40px] text-[#0f092e] sticky top-10 text-center">
        ...
        {showConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#0f092e]/95 backdrop-blur-xl">
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white text-[#0f092e] w-full max-w-lg rounded-[40px] p-10 space-y-6">
              ...
              <div className="text-center py-4 border-y border-gray-100 font-bold text-[11px] uppercase space-y-2">
                <p>{agencyData?.name}</p>
                <p>Commandé par : {selectedUser}</p>
                <p className="text-3xl font-black text-[#0f092e] mt-4">{totalHT.toFixed(2)}€ HT</p>
              </div>
              <button onClick={() => setOrderSent(true)} className="w-full py-6 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-xl">Confirmer et Envoyer</button>
