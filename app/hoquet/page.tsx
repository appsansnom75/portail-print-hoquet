      ...
      <main className="max-w-7xl mx-auto w-full py-16 px-6 pb-40 grid grid-cols-1 md:grid-cols-3 gap-12">
        {products.map((p) => (
          <div key={p.id} className="pt-10 relative group">
            ...
            <div className="bg-white/[0.03] border border-white/10 rounded-[40px] p-8 pt-16 group-hover:border-orange-500/30 transition-all">
              <h3 className={`font-black text-base uppercase mb-6 ${THEME.color}`}>{p.name}</h3>
              <div className="space-y-4">
                {p.hasVariants && <select value={selections[p.id].variant} onChange={(e) => setSelections({...selections, [p.id]:{...selections[p.id], variant: e.target.value}})} className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-[10px] font-black uppercase text-white outline-none focus:border-orange-500 transition-colors">
                  {p.variants.map((v:any)=><option key={v.id} value={v.id}>{v.name}</option>)}
                </select>}
                <select value={selections[p.id].qty} onChange={(e) => setSelections({...selections, [p.id]:{...selections[p.id], qty: Number(e.target.value)}})} className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-[10px] font-black uppercase text-white outline-none focus:border-orange-500 transition-colors">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                  ...
                  <button onClick={() => handleAddToCart(p)} className="bg-white text-[#0f092e] px-6 py-3 rounded-2xl font-black uppercase text-[9px] hover:bg-orange-500 hover:text-white transition-all">Ajouter</button>
                </div>
              </div>
            </div>
          </div>
