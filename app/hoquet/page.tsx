...
const THEME = {
  ...
  shadow: 'group-hover:border-orange-500/30'
};

export default function BusinessPage() {
  // On récupère cart et addToCart du contexte global
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      // Récupère uniquement les produits "Vetements" (Orange)
      ...
      if (data) {
        ...
        // Prépare les choix par défaut (1ère quantité, 1ère variante)
  ...
  const handleAddToCart = (product: any) => {
    ...
    // On récupère le nom de la variante si elle existe
    const variantObj = product.variants.find((v: any) => v.id === sel.variant);
    const variantName = product.hasVariants ? ` - ${variantObj?.name}` : "";

    // AJOUT AU PANIER CORRÉLÉ
    addToCart({
      id: `${product.id}-${sel.variant}`, // ID unique par produit + variante
      name: `${product.name}${variantName}`,
      price: totalPriceHT / Number(sel.qty), // Prix unitaire
      qty: Number(sel.qty), 
      category: THEME.label // "Gamme Business" pour la couleur dans le panier
    });

    setIsCartOpen(true); // Ouvre le volet coulissant
  };

  if (loading) return (
    <div className="min-h-screen bg-[#0f092e] flex items-center justify-center">
       <p className="font-black text-orange-500 tracking-[0.5em] uppercase animate-pulse italic text-xs">Chargement Business...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0f092e] text-white font-sans flex flex-col relative overflow-x-hidden">
      ...
      <header className="py-6 px-6 border-b border-white/10 flex justify-between items-center bg-[#0f092e]/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <button onClick={() => setIsCartOpen(true)} className="relative group">
            <span className="text-[10px] font-black uppercase tracking-widest text-white/40 group-hover:text-orange-500 transition-colors">
              Panier ({cart.length})
            </span>
        </button>
      </header>
      ...
      <main className="max-w-7xl mx-auto w-full py-12 px-6 pb-40">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16">
          {products.map((product) => {
            ...
            // L'image change si la variante a sa propre image
                  ...
                  <div className="space-y-5">
                    {product.hasVariants && (
                       <div className="space-y-2">
                          <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em]">Modèle</p>
                          <select 
                            value={sel.variant} 
                            onChange={(e) => updateSelection(product.id, 'variant', e.target.value)} 
                            className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-[11px] font-black uppercase text-white outline-none focus:border-orange-500 transition-colors cursor-pointer"
                          >
                    ...
                    <div className="flex items-center justify-between pt-6 mt-2 border-t border-white/10">
                      <div>
                        <div className="flex items-baseline gap-1">
                          ...
                          <span className="text-[10px] text-orange-500/60 font-black italic">HT</span>
                        </div>
                        <p className="text-[8px] text-white/20 font-bold uppercase tracking-widest mt-1">Soit {(currentTotal / sel.qty).toFixed(2)}€ / unité</p>
                      </div>
